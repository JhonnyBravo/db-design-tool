...
public class TableDefinitionServiceImpl implements TableDefinitionService {
  ...
  @Override
  public List<TableMaster> findTableByEntityType(int entityType) throws Exception {
    List<TableMaster> recordset = new ArrayList<>();
  ...
  @Override
  public TableDefinition findTableDefinitionByTableId(int tableId) throws Exception {
    ...
    tableDefinition
        .setFieldMaster(fieldMasterList.toArray(new FieldMaster[fieldMasterList.size()]));
  ...
  public void create(TableDefinition tableDefinition) throws Exception {
    ...
    try (SqlSession session = factory.openSession()) {
      ...
      final TableMaster latest = tableMasterRepository.findByLatestTableId();

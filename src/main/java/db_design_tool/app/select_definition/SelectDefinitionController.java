...
public class SelectDefinitionController extends HttpServlet {
  ...
  protected void doGet(HttpServletRequest request, HttpServletResponse response)
    ...
    try {
      ...
      final List<TableMaster> tableMasterList = selectDefinitionService.findEntityAll();
      ...
      try {
        if (!paramMap.containsKey("tableMaster.physicalName")) {
          final SelectDefinition selectDefinition =
              selectDefinitionService.findSelectDefinitionByTableId(Integer.parseInt(tableId));

          request.setAttribute("tableMaster", selectDefinition.getTableMaster());
    ...
    final RequestDispatcher dispatcher =
        context.getRequestDispatcher("/WEB-INF/select_definition/index.jsp");
  ...
  protected void doPost(HttpServletRequest request, HttpServletResponse response)
    ...
    final TableSourceDefinition[] tableSourceDefinitionArray =
        helper.convertToTableSourceDefinition(request);
    ...
    final FieldSourceDefinition[] fieldSourceDefinitionArray =
        helper.convertToFieldSourceDefinition(request);
    ...
    if (tableSourceDefinitionArray != null) {
      for (final TableSourceDefinition tableSourceDefinition : tableSourceDefinitionArray) {
        if (validator.validateTableSourceDefinition(tableSourceDefinition)) {
          hasError = true;
        }
      }
    } else {
      hasError = true;
      request.setAttribute("tableSourceDefinitionError", "結合条件を入力してください。");
    }

    if (fieldSourceDefinitionArray != null) {
      for (final FieldSourceDefinition fieldSourceDefinition : fieldSourceDefinitionArray) {
        if (validator.validateFieldSourceDefinition(fieldSourceDefinition)) {
          hasError = true;
        }
      }
    } else {
      hasError = true;
      request.setAttribute("fieldSourceDefinitionError", "クエリ定義を入力してください。");
    }

    // 登録済みクエリ定義と画面から送信されたフォームデータをマージする。
    if (tableMaster.getTableId() > 0) {
      try {
        final SelectDefinition savedSelectDefinition =
            selectDefinitionService.findSelectDefinitionByTableId(tableMaster.getTableId());

        if (tableSourceDefinitionArray != null) {
          selectDefinition.setTableSourceDefinition(helper.mergeTableSourceDefinition(
              savedSelectDefinition.getTableSourceDefinition(), tableSourceDefinitionArray));
        }

        if (fieldSourceDefinitionArray != null) {
          selectDefinition.setFieldSourceDefinition(helper.mergeFieldSourceDefinition(
              savedSelectDefinition.getFieldSourceDefinition(), fieldSourceDefinitionArray));
          selectDefinition.setFieldMaster(selectDefinition.getFieldSourceDefinition());
        }
      } catch (final Exception e) {
        throw new IOException(e);
      }
    ...
    // 画面へデータを送信する。
    request.setAttribute("tableSourceDefinitionArray", tableSourceDefinitionArray);
    request.setAttribute("fieldSourceDefinitionArray", fieldSourceDefinitionArray);

    final int deleteFlag = Integer.parseInt(request.getParameter("tableMaster.deleteFlag"));

    if (deleteFlag == 1) {
      try {
        selectDefinitionService.deleteByTableId(tableMaster.getTableId());

...
public class SelectDefinitionHelper extends TableDefinitionHelper {
  ...
  public TableSourceDefinition[] mergeTableSourceDefinition(TableSourceDefinition[] reffArray,
    ...
    final List<TableSourceDefinition> reffList = new ArrayList<>(Arrays.asList(reffArray));
    // 削除フラグの設定
    ...
    // 削除されたレコードの抽出
    final List<TableSourceDefinition> deletedList = reffList.stream()
        .filter(tableSourceDefinition -> (tableSourceDefinition.getDeleteFlag() == 1))
  ...
  public FieldSourceDefinition[] mergeFieldSourceDefinition(FieldSourceDefinition[] reffArray,
    ...
    final List<FieldSourceDefinition> reffList = new ArrayList<>(Arrays.asList(reffArray));
    // 削除フラグの設定
    reffList.forEach(reff -> {
      boolean isDeleted = true;
    ...
    // 削除されたレコードの抽出
    final List<FieldSourceDefinition> deletedList = reffList.stream()
        .filter(fieldSourceDefinition -> (fieldSourceDefinition.getDeleteFlag() == 1))
        .collect(Collectors.toList());

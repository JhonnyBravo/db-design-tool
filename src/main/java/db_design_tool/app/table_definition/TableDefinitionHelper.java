...
/**
 * テーブル定義編集画面から送信されたデータの変換処理を管理する。
 */
public class TableDefinitionHelper {
  ...
  public FieldMaster[] mergeFieldMaster(FieldMaster[] reffArray, FieldMaster[] diffArray) {
    ...
    final List<FieldMaster> reffList = new ArrayList<>(Arrays.asList(reffArray));
    // 削除フラグの設定
    ...
    // 削除されたレコードの抽出
    final List<FieldMaster> deletedList = reffList.stream()
        .filter(fieldMaster -> (fieldMaster.getDeleteFlag() == 1)).collect(Collectors.toList());

...
public class TableSourceDefinitionRepositoryTest {
    public static class クエリ定義が存在しない場合 {
        ...
        public static void setUpBeforeClass() throws Exception {
            ...
            final String[] params2 = { "table1", "テーブル1" };
            final TableMaster table1 = helper.createTableMaster(params2);

            final String[] params3 = { "table2", "テーブル2" };
            final TableMaster table2 = helper.createTableMaster(params3);
        ...
        public void findAll実行時に空のリストが返されること() throws Exception {
            try (SqlSession session = factory.openSession()) {
                ...
                final List<TableSourceDefinition> recordset = tableSourceDefinitionRepository.findAll();
        ...
        public void create実行時にレコードを登録できてtrueが返されること() throws Exception {
            ...
            final TableSourceDefinition expect1 = helper.createTableSourceDefinition(params1);
            expectList.add(expect1);

            final String[] params2 = { "1", "2", "3", "condition2" };
            final TableSourceDefinition expect2 = helper.createTableSourceDefinition(params2);
            expectList.add(expect2);

            try (SqlSession session = factory.openSession()) {
                tableSourceDefinitionRepository = session.getMapper(TableSourceDefinitionRepository.class);

                final boolean result = tableSourceDefinitionRepository.create(expectList);
                assertThat(result, is(true));
                session.commit();

                final List<TableSourceDefinition> actualList = tableSourceDefinitionRepository.findAll();
                ...
                assertThat(actual1.getJoinCondition(), is(expect1.getJoinCondition()));
                assertThat(actual1.getDeleteFlag(), is(expect1.getDeleteFlag()));
                ...
                assertThat(actual2.getJoinCondition(), is(expect2.getJoinCondition()));
                assertThat(actual2.getDeleteFlag(), is(expect2.getDeleteFlag()));
            }
        }
    }
    ...
    public static class クエリ定義が存在する場合 {
        ...
        public static void setUpBeforeClass() throws Exception {
            ...
            final SelectDefinitionTestHelper helper = new SelectDefinitionTestHelper();

            final String[] params1 = { "query1", "クエリ1" };
            final TableMaster query1 = helper.createTableMaster(params1);

            final String[] params2 = { "query2", "クエリ2" };
            final TableMaster query2 = helper.createTableMaster(params2);

            final String[] params3 = { "table1", "テーブル1" };
            final TableMaster table1 = helper.createTableMaster(params3);

            final String[] params4 = { "table2", "テーブル2" };
            final TableMaster table2 = helper.createTableMaster(params4);
        ...
        public void setUp() throws Exception {
            ...
            final TableSourceDefinition record1 = helper.createTableSourceDefinition(params1);
            recordset.add(record1);

            final String[] params2 = { "1", "2", "4", "query1.condition2", "1" };
            final TableSourceDefinition record2 = helper.createTableSourceDefinition(params2);
            recordset.add(record2);

            // クエリ2
            final String[] params3 = { "2", "1", "3", "query2.condition1" };
            final TableSourceDefinition record3 = helper.createTableSourceDefinition(params3);
            recordset.add(record3);

            final String[] params4 = { "2", "2", "4", "query2.condition2", "1" };
            final TableSourceDefinition record4 = helper.createTableSourceDefinition(params4);
        ...
        public void create実行時にレコードを登録できてtrueが返されること() throws Exception {
            ...
            try (SqlSession session = factory.openSession()) {
                ...
                final List<TableSourceDefinition> actualList1 = tableSourceDefinitionRepository.findAll();
                assertThat(actualList1.size(), is(6));

                final List<TableSourceDefinition> actualList2 = tableSourceDefinitionRepository.findByTableId(1);
        ...
        public void update実行時にレコードを更新できてtrueが返されること() throws Exception {
            try (SqlSession session = factory.openSession()) {
                ...
                final List<TableSourceDefinition> expectList = tableSourceDefinitionRepository.findByTableId(2);

                final TableSourceDefinition expect1 = expectList.get(0);
                expect1.setNo(2);
                expect1.setSourceId(4);
                expect1.setJoinCondition("query2.update1");

                final TableSourceDefinition expect2 = expectList.get(1);
                expect2.setNo(1);
                expect2.setSourceId(3);
                expect2.setJoinCondition("query2.update2");

                final boolean result = tableSourceDefinitionRepository.update(expectList);
                assertThat(result, is(true));
                session.commit();

                final List<TableSourceDefinition> actualList = tableSourceDefinitionRepository.findByTableId(2);
                assertThat(actualList.size(), is(2));

                final TableSourceDefinition actual1 = actualList.get(0);
                assertThat(actual1.getNo(), is(expect2.getNo()));
                assertThat(actual1.getSourceId(), is(expect2.getSourceId()));
                assertThat(actual1.getJoinCondition(), is(expect2.getJoinCondition()));

                final TableSourceDefinition actual2 = actualList.get(1);
                assertThat(actual2.getNo(), is(expect1.getNo()));
                assertThat(actual2.getSourceId(), is(expect1.getSourceId()));
                assertThat(actual2.getJoinCondition(), is(expect1.getJoinCondition()));
            }
        }

        @Test
        public void deleteByDeleteFlag実行時にdeleteFlagが1であるレコードを削除できてtrueが返されること() throws Exception {
            try (SqlSession session = factory.openSession()) {
                tableSourceDefinitionRepository = session.getMapper(TableSourceDefinitionRepository.class);
                final boolean result = tableSourceDefinitionRepository.deleteByDeleteFlag();
                assertThat(result, is(true));
                session.commit();
